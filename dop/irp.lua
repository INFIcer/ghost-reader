...
local function poll_irp_updates(mark_pending)
  ...
  while processed < IRP_POLL_PER_TICK do
    ...
    if data then
      -- 直接按 unit_number 取 IRP 实体（O(1)），替代全表面 find_entities_filtered
      local irp = game.get_entity_by_unit_number(unit)
      if irp and irp.valid and irp.type ~= "item-request-proxy" then irp = nil end
